...
def process_country_climate_risks(country_file_path, basin_file_path):
    ...
    asset_type = country_file_path.stem.split("_")[0]  # e.g., 'roads' from 'AUT_roads_river_risk.parquet'
    #country_iso3 = country_file_path.stem.split("_")[0]  # e.g., 'AUT' from 'AUT_roads_river_risk.parquet'

    if asset_type == 'roads':
        base_data  = gpd.read_parquet(f"C:\\MIRACA\\infrastructure_data\\{asset_type}_tent.parquet")
    ...
    damage_df = gpd.GeoDataFrame(damage_df.merge(base_data[['id', 'geometry', 'CORRIDORS']], 
                                on=['id', 'CORRIDORS'], 
                                how='left'))
    
    damage_df = damage_df.set_crs(3857).to_crs(3035)
...
def main():
    ...
    files_to_process = [#Path(r"C:\MIRACA\risk\roads_river_TENT_risk.parquet"),
                        Path(r"C:\MIRACA\risk\rail_river_TENT_risk.parquet")
    ]
    skipped_files = []
