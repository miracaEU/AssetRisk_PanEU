...
def load_infrastructure_data(asset_type, standardize_geom=True):
    """
    ...
    if asset_type == 'roads':
        infrastructure_path = Path(r"C:\MIRACA\infrastructure_data\roads_tent.parquet")
    elif asset_type == 'rail':
        infrastructure_path = Path(r"C:\MIRACA\infrastructure_data\rail_tent.parquet")
    
    print(f"Reading feature data for {asset_type}...")
...
def format_risk_results(largest_rp, all_risks, all_exposures, multi_curves, mean_damages, lengths, hazard_type, protection_standard=None):
    ...
    Args:
        ...
        protection_standards (dict, optional): Dictionary mapping id to protection standard
    ...
    if hazard_type == 'river' and protection_standard is not None:
        ...
        largest_rp['protection_standard'] = largest_rp['id'].map(protection_standard)
    ...
    for rp in all_return_periods:
        ...
        if rp in mean_damages:
            largest_rp[f'mean_damage_{rp}'] = largest_rp['id'].apply(lambda x: mean_damages[rp][x] if x in mean_damages[rp].keys() else np.nan)
        
        # Add length
        if rp in lengths:
            largest_rp[f'exposure_{rp}'] = largest_rp['id'].apply(lambda x: lengths[rp][x] if x in lengths[rp].keys() else np.nan)

    # Create final output DataFrame with essential columns
    risk_columns = ['id', 'CORRIDORS', 'NUTS2', 'EAD', 'EAE', 'hazard_type']
...
def process_return_period_wrapper(common_data, args):
    ...
    results['geom_type'] = results['id'].map(
        dict(zip(features['id'], features.geometry.geom_type))
    )    
...
def assess_river_flood_risk(asset_type, nuts2_path=None, 
    ...
    features = load_infrastructure_data(asset_type, standardize_geom)
    ...
    if protection_standard_path:
        ...
        asset_region_ds = dict(zip(protection_standard.id.values, protection_standard.design_standard.values))
    else:
   ...
   # Package the common data that doesn't change per return period
    ...
    for return_period, result_df in results:
        ...
        mean_damages[return_period] = dict(zip(result_df['id'], result_df[mean_damage_col]))
        lengths[return_period] = dict(zip(result_df['id'], result_df[length_col]))
    ...
    # Create lookup for id
    lookup_id = dict(zip(features.index, features.id))
    ...
    for curve in tqdm(multi_curves.keys(),total=len(multi_curves), desc="Calculating risks per curve"):
        ...
        for idx, row in subrisk.iterrows():
            ...
            design_standard = asset_region_ds.get(lookup_id[idx], 0)
            ...
            for rp in RP_list:
                ...
                try:
                    ...
                    rp_lengths[rp] = lengths[rp][lookup_id[idx]]
                except:
                    rp_lengths[rp] = 0
...
def assess_coastal_flood_risk(asset_type, nuts2_path, 
    ...
    for flood_item in list(collection.get_items()):
        ...
        if len(collect_damages) > 0:
            ...
            combined_damage = combined_damage.groupby('id', as_index=False).agg(agg_dict)
...
def assess_windstorm_risk(asset_type, nuts2_path=None, 
...
def miraca_risk(asset_type, hazard_type, 
    ...
    print(f"Starting MIRACA risk assessment for {asset_type}, {hazard_type}")
...
if __name__ == "__main__":
    hazard_type = 'river' #sys.argv[2]  # e.g., 'river', 'coastal', or 'both'
    asset_type = 'rail' #sys.argv[1]  # e.g., 'rail', 'roads', 'power', 'water'
    ...
    print(f"\nProcessing {asset_type} - {hazard_type}")             # Add command line arguments for coastal scenarios if needed
    ...
    result_file = result_dir / f"{asset_type}_{hazard_type}{scenario_suffix}_TENT_risk.parquet"
    ...
    print(f"Running risk assessment for {asset_type} with {hazard_type} hazard...")
    ...
    if len(risk_results) > 0:
        ...
        print(f"No risk results found for {asset_type} with {hazard_type} hazard. Skipping save.")
