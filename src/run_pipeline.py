...
def run_pipeline(
    ...
    work_items = [
        ...
        if (
            config.EXPOSURE_DIR / _folder(asset) / f"{_folder(asset)}_{country}.parquet"
        ).exists()
    ]
    ...
    print(
        f"Workers:     {effective_workers} outer"
    ...
    if not work_items:
        print(
            "No work items found. Check exposure directory and country/asset filters."
        )
        return
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=n_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            try:
                ...
                results.append(
                    {"label": f"{country}/{asset}", "status": "error", "error": str(e)}
                )
